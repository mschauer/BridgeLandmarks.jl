...
p_ahs = Pars_ahs(δmom=0.01, cinit=1.0, γinit=1.0, db=[4.0,3.0],stdev=2.0, σobs = 0.1)
...
# run algorithm
@time landmarksmatching(xobs0,xobsT; ITER=50,pars=p_ms, outdir=outdir, ainit=ainit, updatescheme=ups)
@time landmarksmatching(xobs0,xobsT; ITER=300,pars=p_ahs, outdir=outdir, ainit=ainit, updatescheme=ups)
