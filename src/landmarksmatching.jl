...
function landmarkmatching(
    ...
    elseif model == :ahs
        cinit = pars.cinit_ahs
        γinit = pars.γinit_ahs
        stdev = pars.stdev_ahs
        nfsinit = construct_nfs(pars.db_ahs, stdev, γinit)
        P = Landmarks(ainit, cinit, n, pars.db_ahs , stdev, nfsinit)
    end
