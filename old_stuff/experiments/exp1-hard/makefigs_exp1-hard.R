...
parsdf$par <- factor(ppar1$par, levels=c('a', 'c', 'gamma'), labels=c("a","c",expression(gamma)))

tracepars <- parsdf %>% gather(key=par, value=value, a, c, gamma)  %>%
  ggplot(aes(x=iterate, y=value)) + geom_path() + geom_point(size=0.5) + facet_wrap(~par, scales="free_y",ncol=1,labeller = label_parsed) +
  xlab("iterate") + ylab("")
tracepars
